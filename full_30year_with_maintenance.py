#!/usr/bin/env python3
"""
30-Year Rent vs Buy Analysis WITH MAINTENANCE COSTS
Simple year-by-year net worth comparison including all inflating costs
"""
...
# Maintenance costs (inflation-adjusted each year)
ROOF_COST_INITIAL = 25_000
ROOF_FREQUENCY = 25  # years
WATER_HEATER_COST_INITIAL = 3_000
WATER_HEATER_FREQUENCY = 10  # years
FLOORING_COST_INITIAL = 10_000
FLOORING_FREQUENCY = 10  # years
...
def calculate_maintenance_cost(year):
    """Calculate maintenance costs for a given year with inflation"""
    total = 0

    # Roof replacement (every 25 years)
    if year % ROOF_FREQUENCY == 0:
        inflated_cost = ROOF_COST_INITIAL * ((1 + INFLATION_RATE) ** (year - 1))
        total += inflated_cost

    # Water heater (every 10 years)
    if year % WATER_HEATER_FREQUENCY == 0:
        inflated_cost = WATER_HEATER_COST_INITIAL * ((1 + INFLATION_RATE) ** (year - 1))
        total += inflated_cost

    # Flooring (every 10 years)
    if year % FLOORING_FREQUENCY == 0:
        inflated_cost = FLOORING_COST_INITIAL * ((1 + INFLATION_RATE) ** (year - 1))
        total += inflated_cost
...
# Track cumulative maintenance
cumulative_maintenance_3 = 0
cumulative_maintenance_20 = 0

print("="*80)
print("30-YEAR NET WORTH COMPARISON (WITH MAINTENANCE COSTS)")
print("="*80)
...
print(f"\nMaintenance Costs (inflation-adjusted):")
print(f"  Roof: ${ROOF_COST_INITIAL:,} every {ROOF_FREQUENCY} years")
print(f"  Water Heater: ${WATER_HEATER_COST_INITIAL:,} every {WATER_HEATER_FREQUENCY} years")
print(f"  Flooring: ${FLOORING_COST_INITIAL:,} every {FLOORING_FREQUENCY} years")
print()

# Print header
print(f"{'Year':<6} {'Renting':>15} {'3% Down':>15} {'20% Down':>15} {'Maint 3%':>12} {'Maint 20%':>12}")
print("="*80)
...
for year in range(1, 31):
    ...
    # Calculate maintenance for this year
    maintenance_cost = calculate_maintenance_cost(year)
    ...
    total_costs_3 = (monthly_payment_3pct * 12) + property_tax + home_insurance + pmi_3 + maintenance_cost
    cumulative_maintenance_3 += maintenance_cost
    ...
    # Grow investments, then subtract maintenance (one-time cost paid from cash)
    investment_3pct, _, _ = calculate_investment_growth_with_contributions(
        investment_3pct, 0, 12)
    investment_3pct -= maintenance_cost  # Pay maintenance from investment account
    ...
    total_costs_20 = (monthly_payment_20pct * 12) + property_tax + home_insurance + maintenance_cost
    cumulative_maintenance_20 += maintenance_cost
    ...
    investment_20pct -= maintenance_cost  # Pay maintenance from investment account
    ...
    # Print year results
    maint_str_3 = f"${maintenance_cost:>10,.0f}" if maintenance_cost > 0 else "-"
    maint_str_20 = f"${maintenance_cost:>10,.0f}" if maintenance_cost > 0 else "-"
    print(f"{year:<6} ${net_worth_rent:>14,.0f} ${net_worth_3:>14,.0f} ${net_worth_20:>14,.0f} {maint_str_3:>12} {maint_str_20:>12}")
...
print(f"\nTOTAL MAINTENANCE COSTS PAID (30 years):")
print(f"  3% Down:   ${cumulative_maintenance_3:>14,.0f}")
print(f"  20% Down:  ${cumulative_maintenance_20:>14,.0f}")
